...
type Repository interface {
	...
	Store(context.Context, int, time.Duration) *StoreReport
	...
	DNS(ctx context.Context, address string, fallback string, records *store.Record) *DNSReport
	...
	HTTP(ctx context.Context, port int) *HTTPReport
	...
	Merge(context.Context, *StoreReport, *DNSReport, *HTTPReport) *Report
}
